...
## Implementation Plan
1. **index.html:** Teachable Machine, TensorFlow.js, html2canvas 라이브러리 추가. 결과 카드 및 저장 버튼 구조 추가. 다크 모드 토글 버튼 추가.
2. **style.css:**
    - CSS Variables를 활용한 테마(`:root`, `body.dark-mode`) 스타일 정의.
    - 결과 카드(`share-card`), 연예인 칩, 저장 버튼 스타일 추가.
3. **main.js:**
    - `animalDetails` 객체에 연예인 리스트 데이터 추가.
    - 다크 모드 토글 로직 및 `localStorage` 연동.
    - `displayResults` 함수 수정:
        - 결과 카드에 사용자 이미지 복제.
        - 연예인 리스트 렌더링.
    - `saveResultImage` 함수 구현: `html2canvas`를 사용하여 `share-card` 영역 캡처 및 다운로드 (다크 모드 배경색 반영).
